...
\begin{center}
  ...
  \vspace{1.2cm}
  \begin{tikzpicture}
    \node[draw=AccentCyan!80, fill=LightBg, thick, rounded corners=5pt, inner xsep=12pt, inner ysep=8pt, font=\small, drop shadow={fill=black!10, opacity=0.4, shadow xshift=1.5pt, shadow yshift=-1.5pt}] {
      \textbf{\color{PrimaryBlue}GitHub Repository:}\quad 
      \href{https://github.com/pkelotra/NO_SQL-Project}{\color{AccentTeal}\texttt{github.com/pkelotra/NO\_SQL-Project}}
    };
  \end{tikzpicture}
\end{center}
...
\begin{itemize}
  \item \textbf{Mapper:} Reads raw HDFS splits and applies \texttt{LogParser.java}. Valid records emit composite \texttt{Text} keys (e.g., date \& hour) and values (e.g., status \& host). It tracks data quality dynamically using Hadoop Counters (\texttt{MALFORMED} vs \texttt{VALID\_RECORDS}).
  \item \textbf{Reducer:} Aggregates grouped keys iteratively. For example, \texttt{HourlyErrorMR} loops through values to count total requests, isolate errors (status 400--599), and uses a Java \texttt{HashSet} for distinct host tracking before calculating the final error rate.
  \item \textbf{Driver:} Configures the job, triggers execution, reads the final HDFS \texttt{part-*} output files sequentially using a \texttt{BufferedReader}, and commits the aggregated results to PostgreSQL via DAOs (e.g., \texttt{Q3DAO.saveResult()}) before cleaning up the temporary HDFS directory.
\end{itemize}
\end{mrbox}
...
\textbf{Execution Model:}
\begin{itemize}
  \item \textbf{Ingestion:} The driver uses a \texttt{BufferedReader} to process the local file line-by-line via \texttt{LogParser.java}. Valid lines are converted into BSON \texttt{Document} objects and loaded into the database using an efficient \texttt{collection.insertMany()} bulk operation.
  \item \textbf{Aggregation:} Queries are executed via the MongoDB Java Driver's declarative aggregation pipeline (\texttt{collection.aggregate()}). For example, \texttt{Q1Mongo} uses \texttt{Aggregates.group()} combined with \texttt{Accumulators.sum()} to compute request counts and total bytes directly within the database engine.
  \item \textbf{Result Extraction:} An \texttt{AggregateIterable<Document>} cursor iterates over the computed BSON outputs, extracts the metrics, and delegates permanent storage to PostgreSQL via DAOs (e.g., \texttt{Q1DAO.saveResult()}).
\end{itemize}
\end{mongobox}
...
\textbf{Execution Model:} 
\textit{[To be updated based on Hive implementation]}
\begin{itemize}
  \item \textbf{Ingestion:} \dots
  \item \textbf{Aggregation:} \dots
  \item \textbf{Result Extraction:} \dots
\end{itemize}
\end{hivebox}
...
\textbf{Execution Model:} 
\textit{[To be updated based on Pig implementation]}
\begin{itemize}
  \item \textbf{Ingestion:} \dots
  \item \textbf{Aggregation:} \dots
  \item \textbf{Result Extraction:} \dots
...
execution\_id        & INTEGER      & Identifier grouping batches of one full pipeline run \\
pipeline\_name       & VARCHAR(20)  & Execution engine (e.g., \texttt{mapreduce}, \texttt{mongodb}) \\
batch\_id            & INTEGER      & Sequential batch identifier (starts at 1) \\
batch\_size          & INTEGER      & Configured number of records per batch \\
avg\_batch\_size     & FLOAT        & Calculated average batch size across the dataset \\
runtime              & FLOAT        & Wall-clock time in seconds (ETL only) \\
malformed\_record\_count & INTEGER  & Number of log lines that failed regex parsing \\
dataset\_name        & VARCHAR      & The name or path of the selected dataset \\
...
hosts\_list        & TEXT       & Comma-separated list of host IP addresses \\
\bottomrule
\end{tabularx}
\end{schematable}
